...
fn test_nft() {
    ...
    // fail: send-nftA tries to send token0 of nftA to send-nftB,
    // but send-nftB doesn't allow that nftA contract yet
    {
        // fail: send-nftA tries to send token0 of nftA to send-nftB,
        // but send-nftB doesn't allow that nftA contract yet
        ...
        assert!(res
            .all_logs()
            .contains(&"Return token token-0 from @send-nft-b to @send-nft-a".to_string()));
        let transfer_success = res.unwrap_json::<bool>();
        assert!(!transfer_success);

        // ok: confirm that send-nftA owns token0 on nftA
        let res = view!(nftA.nft_token(tokens[0].into()));
        assert_eq!(
            res.unwrap_json::<nft::Token>().owner_id,
            send_nftA.account_id()
        ...
        // ok: confirm that user1 owns token0 on send-nftA
        let res = view!(send_nftA.get_tokens_owned_by_users(nftA.account_id().into(), None, None));
        assert_eq!(
            res.unwrap_json::<Vec<(nft::TokenId, AccountId)>>(),
            vec![(tokens[0].to_string(), users[1].account_id())]
    ...
    // ok: root mints token1 for user0 on nftA
    {
        let res = call!(
            &root,
            nftA.nft_mint(tokens[1].into(), user(0), utils::token_metadata()),
            deposit = parse_near!("5660 microN")
        );
        print_vec(&res.all_logs());
        res.pretty_debug();
        res.assert_success();
    ...
    // ok: user0 sends the token1 to send-nft on nftA
    {
        // ok: user0 sends the token0 to send-nft
        let tags = NearAppsTags::new("nftA", 3, "user0");
        let res = call!(
        ...
        // ok: confirm that send-nft owns token1 on nftA
        let res = view!(nftA.nft_token(tokens[1].into()));
        assert_eq!(
            res.unwrap_json::<nft::Token>().owner_id,
            send_nftA.account_id()
        );

        // ok: confirm that user0 owns token1 on send-nft
        let res = view!(send_nftA.get_tokens_owned_by_users(nftA.account_id().into(), None, None));
        print_vec(&res.all_logs());
        assert_eq!(
            res.unwrap_json::<Vec<(nft::TokenId, AccountId)>>(),
            vec![(tokens[1].to_string(), users[0].account_id())]
        );
    }
}
